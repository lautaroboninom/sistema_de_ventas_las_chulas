...
SQL_STATEMENTS = [
    ...
    """
    DROP VIEW IF EXISTS vw_model_hierarchy_detail""",
    """
    ...
    """
    INSERT INTO marca_series_variantes (marca_id, tipo_id, serie_id, nombre)
    SELECT @bmc_id, @tipo_bpap, @serie_auto_g3, v.nombre
...
def apply_catalog_hierarchy_patch(apps, schema_editor):
    if schema_editor.connection.vendor != "mysql":
        return
    with schema_editor.connection.cursor() as cursor:
        for raw in SQL_STATEMENTS:
            stmt = raw.strip()
            if not stmt:
                continue
            cursor.execute(stmt)
...
class Migration(migrations.Migration):
    ...
    dependencies = []
    operations = [
        migrations.RunPython(apply_catalog_hierarchy_patch, migrations.RunPython.noop),
    ]
