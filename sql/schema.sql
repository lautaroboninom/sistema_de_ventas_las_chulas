...
-- Usuario tecnico de prueba (solo desarrollo local)
INSERT INTO users(nombre, email, hash_pw, rol, activo)
VALUES (
  'Tecnico Las Chulas',
  'tecnico@laschulas.locas',
  'argon2$argon2id$v=19$m=102400,t=2,p=8$c1FZTU8xNXV3R3IzMDZ5Z2t1dEc1MA$aWYn7KZn0eD0c6DQ+ee6t6NlNZG5ra3CmVjHToJTsIQ',
  'admin',
  TRUE
)
ON CONFLICT (email) DO UPDATE
...
CREATE TABLE IF NOT EXISTS retail_settings (
  ...
  arca_cbte_tipo_store          INTEGER NOT NULL DEFAULT 6,
  arca_cbte_tipo_online         INTEGER NOT NULL DEFAULT 6,
  arca_cert_path                TEXT,
  arca_key_path                 TEXT,
  ...
  CONSTRAINT chk_retail_settings_cbte_store CHECK (arca_cbte_tipo_store > 0),
  CONSTRAINT chk_retail_settings_cbte_online CHECK (arca_cbte_tipo_online > 0),
...
ALTER TABLE retail_settings
ADD COLUMN IF NOT EXISTS arca_cbte_tipo_store INTEGER;
...
UPDATE retail_settings
SET arca_cbte_tipo_store = COALESCE(arca_cbte_tipo_store, 6),
    arca_cbte_tipo_online = COALESCE(arca_cbte_tipo_online, COALESCE(arca_cbte_tipo_store, 6))
WHERE id = 1;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_retail_settings_cbte_store'
  ) THEN
    ALTER TABLE retail_settings
      ADD CONSTRAINT chk_retail_settings_cbte_store CHECK (arca_cbte_tipo_store > 0);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_retail_settings_cbte_online'
  ) THEN
    ALTER TABLE retail_settings
      ADD CONSTRAINT chk_retail_settings_cbte_online CHECK (arca_cbte_tipo_online > 0);
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS retail_cash_session_movements (
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT chk_cash_movement_type CHECK (movement_type IN ('opening', 'sale', 'return', 'expense', 'income', 'manual_adjustment', 'exchange_settlement', 'closing')),
  CONSTRAINT chk_cash_direction CHECK (direction IN ('in', 'out')),
  CONSTRAINT chk_cash_movement_method CHECK (payment_method IS NULL OR payment_method IN ('cash', 'debit', 'transfer', 'credit', 'store_credit')),
...
CREATE TABLE IF NOT EXISTS retail_sales (
  ...
  CONSTRAINT chk_retail_sales_payment_method CHECK (payment_method IN ('cash', 'debit', 'transfer', 'credit', 'store_credit')),
...
CREATE TABLE IF NOT EXISTS retail_sale_payments (
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT chk_sale_payment_method CHECK (payment_method IN ('cash', 'debit', 'transfer', 'credit', 'store_credit')),
...
CREATE TABLE IF NOT EXISTS retail_invoices (
  id                  BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  sale_id             BIGINT NOT NULL REFERENCES retail_sales(id) ON DELETE CASCADE,
  arca_account_id     BIGINT REFERENCES retail_arca_accounts(id) ON DELETE RESTRICT,
...
DECLARE invoice_default_pto INTEGER;
DECLARE invoice_default_cbte INTEGER;
BEGIN
  SELECT id
  ...
  IF legacy_account_id IS NULL THEN
    ...
    INTO invoice_default_pto, invoice_default_cbte
