...
import api, { getAccesoriosCatalogo } from "../lib/api";
...
export default function Accesorios() {
  ...
  async function loadAccesorios() {
    setErr(""); setMsg("");
    try { setItems(await getAccesoriosCatalogo()); } catch (e) { setErr(e.message || "No se pudo cargar"); setItems([]); }
  }

  useEffect(() => { loadAccesorios(); }, []);
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      setErr("");
      setMsg("");
      await api.post("/api/catalogos/accesorios/", { nombre });
      setForm({ nombre: "" });
      await loadAccesorios();
      setMsg("Accesorio agregado");
    } catch (e) {
  ...
  async function renameItem(it) {
    ...
    try {
      ...
      setErr("");
      setMsg("");
      await api.post("/api/catalogos/accesorios/", { rename_from: it.nombre, nombre });
      await loadAccesorios();
      setMsg("Accesorio renombrado");
    } catch (e) {
  ...
  async function removeItem(it) {
    ...
    try {
      ...
      setErr("");
      setMsg("");
      await api.del(`/api/catalogos/accesorios/?nombre=${encodeURIComponent(it.nombre)}`);
      await loadAccesorios();
      setMsg("Eliminado");
    } catch (e) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-3">Accesorios</h1>
      ...
      <form onSubmit={handleAdd} className="border rounded p-3 mb-3 flex gap-2 items-end">
        <div className="flex-1">
          <label className="text-sm block mb-1">Nuevo accesorio</label>
          <Input
