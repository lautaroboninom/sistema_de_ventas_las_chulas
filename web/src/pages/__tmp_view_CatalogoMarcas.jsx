...
export default function CatalogoMarcas() {
  ...
  const [fmo, setFmo] = useState({ nombre: "", tipo_equipo: "", tecnico_id: "", variante: "" });
  ...
  const [mdlVarSel, setMdlVarSel]   = useState({}); // { [modeloId]: variante }
  ...
  const loadModelos = async (brandId) => {
    setModelos([]); setMdlTecSel({}); setMdlTipoSel({}); setMdlVarSel({});
    if (!brandId) return;
    try {
      const ms = await getModelos(brandId); // debe incluir campos tipo_equipo y variante
      setModelos(ms);
      const mapTec = {}; const mapTipo = {}; const mapVar = {};
      (ms || []).forEach((m) => {
        mapTec[m.id] = m.tecnico_id ?? "";
        mapTipo[m.id] = m.tipo_equipo ?? "";
        mapVar[m.id] = m.variante ?? "";
      });
      setMdlTecSel(mapTec); setMdlTipoSel(mapTipo); setMdlVarSel(mapVar);
  ...
  // (EliminaciÃ³n simple de marca removida a pedido. Solo queda cascada)
  // EliminaciÃ³n en cascada (marca + TODOS sus modelos)
  const delMarcaCascade = async (id) => {
    const typed = prompt(
      "Esta acciÃ³n eliminarÃ¡ la marca y TODOS sus modelos. Se desvincularÃ¡n equipos relacionados.\nEscribe ELIMINAR para confirmar:",
      ""
    );
    if (typed !== "ELIMINAR") return;
    try {
      setErr(""); setMsg("");
      await deleteMarcaCascade(id);
      setSel(null);
      loadMarcas();
      setMsg("Marca y modelos eliminados");
    } catch (e) {
      setErr(e?.message || "No se pudo eliminar en cascada");
    }
  };
  ...
  const addModelo = async (e) => {
    ...
    try {
      await postModelo(sel.id, {
        ...
        variante: (fmo.variante || "").trim() || undefined,
      });
      setFmo({ nombre: "", tipo_equipo: "", tecnico_id: "", variante: "" });
  ...
  async function guardarTecnicoMarcaYApli() {
    if (!sel) return;
    if (!marcaTecId) {
      if (!confirm("Vas a dejar la marca sin tÃ©cnico. Â¿Continuar?")) return;
    } else {
      if (!confirm("Aplicar este tÃ©cnico a TODOS los modelos de la marca (sobrescribe). Â¿Continuar?")) return;
    }
    try {
      setLoading(true); setErr(""); setMsg("");
      await patchMarcaTecnico(sel.id, marcaTecId ? Number(marcaTecId) : null);
      await postMarcaAplicarTecnico(sel.id, true);
      setMsg("TÃ©cnico de marca guardado y aplicado a todos los modelos.");
      setSel(prev => prev ? { ...prev, tecnico_id: (marcaTecId || null) ? Number(marcaTecId) : null } : prev);
      await loadModelos(sel.id);
    } catch (e) {
      setErr(e.message || "No se pudo asignar/aplicar el tÃ©cnico de la marca");
  ...
  async function guardarTecnicoModelo(modelId) {
    ...
    try {
      ...
      setMsg("TÃ©cnico del modelo guardado.");
      setModelos(ms => ms.map(m => m.id === modelId ? { ...m, tecnico_id: tId ? Number(tId) : null } : m));
    } catch (e) {
      setErr(e.message || "No se pudo guardar el tÃ©cnico del modelo");
    } finally { setLoading(false); }
  }
  ...
  async function guardarVariante(modelId) {
    const varianteText = (mdlVarSel[modelId] || "").trim();
    try {
      setLoading(true); setErr(""); setMsg("");
      await patchModeloVariante(sel.id, modelId, { variante: varianteText });
      setMsg("Variante del modelo guardada.");
      setModelos(ms => ms.map(m => m.id === modelId ? { ...m, variante: varianteText } : m));
    } catch (e) {
      setErr(e.message || "No se pudo guardar la variante");
    } finally { setLoading(false); }
  }
              ...
              <li key={m.id} className={`p-2 ${sel?.id === m.id ? "bg-gray-50" : ""}`}>
                <div className="flex items-center justify-between">
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (listRef.current) { try { listRef.current.scrollTop = 0; } catch {} }
                    }}
                  >
                    {m.nombre}
                  </button>
                  <div className="flex gap-2">
                    <button className="px-2 py-1 border rounded" onClick={async () => {
                      const nuevo = prompt("Renombrar marca", m.nombre || "");
                      const nombre = (nuevo || "").trim();
                      if (!nombre || nombre === m.nombre) return;
                      try {
                        setErr(""); setMsg("");
                        await patchMarca(m.id, { nombre });
                        setMarcas(arr => arr.map(x => x.id === m.id ? { ...x, nombre } : x));
                        if (sel?.id === m.id) setSel(prev => prev ? { ...prev, nombre } : prev);
                        setMsg("Marca renombrada");
                      } catch (e) { setErr(e.message || "No se pudo renombrar la marca"); }
                    }}>Renombrar</button>
                ...
                {sel?.id === m.id && (
                  <div className="mt-3 border-t pt-3">
                    <label className="text-sm block mb-1">TÃ©cnico asignado a la marca</label>
      ...
      <div>
        ...
        {sel && (
          <form onSubmit={addModelo} className="border rounded p-3 mb-3 grid grid-cols-1 md:grid-cols-5 gap-2 items-end">
            ...
            <div>
              <label className="text-sm block mb-1">TÃ©cnico</label>
        ...
        <ul className="border rounded divide-y">
          {sel ? (
              ...
              <li key={md.id} className="p-2">
                ...
                <div className="flex items-center justify-between">
                  <button
                    ...
                    onClick={() => {
                      const expanding = expandedModelId !== md.id;
                      setExpandedModelId(expanding ? md.id : null);
                    }}
                  >
                    {md.nombre}
                  </button>
                  <div className="text-xs text-gray-600 flex gap-3 items-center">
                    ...
                    <span>Var.: {md.variante || "-"}</span>
                    <span>TÃ©c.: {md.tecnico_id ? (tecnicos.find((t) => t.id === md.tecnico_id)?.nombre || md.tecnico_id) : "hereda/ninguno"}</span>
                  </div>
                </div>
                ...
                {expandedModelId === md.id && (
                  <div className="mt-3 border rounded p-3 grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-gray-50">
                    ...
                    <div className="md:col-span-4">
                      <label className="text-sm block mb-1">Variante</label>
                      <Input value={mdlVarSel[md.id] ?? (md.variante || "")} onChange={(e) => setMdlVarSel((s) => ({ ...s, [md.id]: e.target.value }))} placeholder="p.ej. 8L" />
                    </div>
                    ...
                    <div className="md:col-span-12 flex gap-2 justify-end mt-3">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            setErr(e.message || "No se pudo renombrar el modelo");
                          } finally { setLoading(false); }
                        }}
                        type="button"
                      >
                      ...
                      <button className="px-3 py-2 border rounded disabled:opacity-60" onClick={() => guardarVariante(md.id)} type="button" disabled={loading} title="Guardar variante">Guardar variante</button>
                      <button className="px-3 py-2 border rounded disabled:opacity-60" onClick={() => guardarTecnicoModelo(md.id)} type="button" disabled={loading}>Guardar tÃ©cnico</button>
            ...
            <li className="p-3 text-center text-gray-500">SeleccionÃ¡ una marca</li>
          )}
